    ...
    <footer className="main-footer fixed bottom-0 w-full z-[-1]">
      <div className="main-container">
          ...
          <div className="main-footer__row main-footer__row-2">
            ...
            <P size="medium" className="main-footer__short-desc leading-8">
              <a href={`mailto:${dataPage.email}`} className="block">
                {t("email")}: <span className="underline">{dataPage.email}</span>
              </a>
              <a href={`tel:+593${dataPage.telefono}`} className="block">
                {t("phone")}: <span>(+593) {dataPage.telefono}</span>
              </a>
              <a
                target="_blank"
                href={`https://api.whatsapp.com/send?phone=+593${dataPage.telefono}&text=${encodeURIComponent(t("whatsappMessage"))}`}
                className="block"
              >
                {t("whatsapp")}: <span>(+593) {dataPage.telefono}</span>
              </a>
            </P>
          </div>
        </div>
        ...
        <div className="main-footer__lower">
          © Copyright {fullYear}. {t("copyright")}
